...
async function main(): Promise<void> {
  ...
  // Inlined rather than read from the server's config: `shared/` is imported by
  // both halves of the product and must not reach into either, or the relay
  // would pull the whole scenario layer in behind one default path. This is the
  // same expression `loadConfig` evaluates, and `SCENARIOS_DIR` still wins.
  const root = arg
    ? resolve(arg)
    : resolve(process.env.SCENARIOS_DIR ?? join(import.meta.dirname, '..', '..', 'scenarios'));
